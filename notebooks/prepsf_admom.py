...
def _deweight_moments(Irr, Irc, Icc, Wrr, Wrc, Wcc, minT, deltaT):
    ...
    if detm <= GMIX_LOW_DETVAL:
        flags = -1
        Wrr, Wrc, Wcc = e2mom(0, 0, minT)
        return flags, Wrr, Wrc, Wcc
    ...
    if wT <= minT:
        ...
        flags = 0
        we1, we2, wT = _scale_e1e2T(we1, we2, wT, minT, deltaT)
        Wrr, Wrc, Wcc = e2mom(we1, we2, wT)
...
class PrePSFAdmom(object):
    ...
    """
    def __init__(
        self, min_fwhm, delta_fwhm, pad_factor=4, ap_rad=1.5,
        maxiter=DEFAULT_MAXITER, etol=DEFAULT_ETOL, Ttol=DEFAULT_TTOL,
        rng=None,
    ):
    ...
    def go(self, obs, guess, return_kernels=False, no_psf=False):
        ...
        guess : float
            A guess for the kernel FWHM. The other parameters are generated.
        ...
        return self._run_admom(obs, psf_obs, return_kernels, d, fwhm_to_T(guess))

    def _run_admom(self, obs, psf_obs, return_kernels, d, guessT):
        ...
        for i in range(self.maxiter):
            ...
            if (
                ...
                & _comp_rel_err(Irr, Irrold, self.Ttol)
                & _comp_rel_err(Irc, Ircold, self.Ttol)
            ):
                am_flags = 0
                break
            else:
                ...
                Irrold = Irr
                Ircold = Irc
                am_flags, wrr, wrc, wcc = _deweight_moments(
                    Irr, Irc, Icc, wrr, wrc, wcc, minT, deltaT
                )
                if am_flags == -1:
                    res, kernels = self._meas_mom(wrr, wrc, wcc, obs, d)
                    am_flags = 0
                    logger.debug("pre-psf adaptive moments hit min fwhm")
                    break
    ...
    def _meas_mom(self, wrr, wrc, wcc, obs, d):
        ...
        return make_mom_result(mom, mom_cov), kernels
